...
const beginnerConfig: TrailConfig = { routeType: "beginner", tier: 0, totalTiers: 8 };
const normalConfig: TrailConfig = { routeType: "route", tier: 3, totalTiers: 8 };
const eliteConfig: TrailConfig = { routeType: "elite_route", tier: 4, totalTiers: 8 };
const championConfig: TrailConfig = { routeType: "champion", tier: 7, totalTiers: 8 };
...
describe("trail generation", () => {
  ...
  test("elite routes are longer than normal routes at same tier", () => {
    ...
    const normal = generateTrail({ routeType: "route", tier: 4, totalTiers: 8 }, rng1);
    const elite = generateTrail({ routeType: "elite_route", tier: 4, totalTiers: 8 }, rng2);
  ...
  test("higher tier routes are longer", () => {
    const rng1 = seededRng();
    const rng2 = seededRng();
    const early = generateTrail({ routeType: "route", tier: 0, totalTiers: 8 }, rng1);
    const late = generateTrail({ routeType: "route", tier: 6, totalTiers: 8 }, rng2);
  ...
  test("higher tier routes have higher max VP per spot", () => {
    const rng1 = seededRng();
    const rng2 = seededRng();
    const early = generateTrail({ routeType: "route", tier: 1, totalTiers: 8 }, rng1);
    const late = generateTrail({ routeType: "route", tier: 6, totalTiers: 8 }, rng2);
